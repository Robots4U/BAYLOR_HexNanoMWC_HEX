...
unsigned long fireTime = 0;    // Store time led was activated
...
unsigned long timeToLaunch = 1500000;  //time in microseconds
...
void go_disarm() {
  ...
  if (f.ARMED) {
    f.ARMED = 0;
    // hits = 0; // reset the number of hits to the default
#ifdef LOG_PERMANENT
...
void loop () {
  ...
  // this code was used to see what values were being read into the serial monitor from the IR sensor
  ...
  /* This portion of code will register a "hit" if the value in the serial monitor goes above 50. */
  ...
  /*  Prior to the end of the semester we had been testing two different ways to do the hit detection. One method utilized a
      switch statement and the other an if...elseif. This current version of the code will simply casue the RGB LED to change
      color upon being hit.
  */

  // Method One: switch case
  ...
  if (isFiring && ((currentTime - fireTime) < 10000)) {
    FIRE_PIN_ON;
  } else {
    FIRE_PIN_OFF;
    fireTime = 0;
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    if (rcOptions[BOXBARO] || launchCalledHover) {  // Baylor Added launchCalledHover to enable it after launch
      if (!f.BARO_MODE) {
        f.BARO_MODE = 1;
        if (!launchCalledHover) {
          AltHold = EstAlt;
        }
  ...
  /*
    String strBaro = "BaroAlt: " + (String)BaroAlt;
    String strEst = "EstAlt: " + (String)EstAlt;
    String strAltHold = "AltHold " + (String)AltHold;
    String strBaroPID = "BaroPID: " + (String)BaroPID;

    Serial.println(strBaro);
    Serial.println(strEst);
    Serial.println(strAltHold);
    Serial.println(strBaroPID);
  */
...
void setBaylorPIDs() {
  //Alt Hold PID
  conf.P8[PIDALT] = 60;  //Best 60
  conf.I8[PIDALT] = 25;  //Best 25
  conf.D8[PIDALT] = 24;  //Best 22  Lower Up Higher Down
